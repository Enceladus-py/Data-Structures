...
{
  // **********************Fill this method
  Team *copy_tarr = new Team[current_size+1]; // copy of old array with +1 length
  for(int i=0;i<current_size;i++){
      copy_tarr[i] = tarr[i]; // copying old contents
  }
  copy_tarr[current_size].setName(team_name); //set name and id for new team
  copy_tarr[current_size].setId(current_size);
  delete[] tarr;
  tarr = copy_tarr;  // old = new
  current_size++;
  return;
};
