...
class PriorityQueue{
    public:
        Node * head;
        void push(Node*);
        bool contains(string); //checks if queue contains the symbol or not
        void swap_nodes(Node**, Node**); //swaps the datas of nodes in queue
        int token_count;
        int changing_tokens; // tokens whose amount can be decreased
        PriorityQueue();
        Node *pop();
};
